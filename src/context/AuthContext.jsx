...
export function AuthProvider({ children }) {
  ...
  const login = async (email, motDePasse) => {
    ...
    localStorage.setItem('jpg_refresh_token', data.refreshToken);
  ...
  const logout = () => {
    ...
    // Révoque le refresh token côté serveur — sans ça, un token volé avant la
    // déconnexion resterait utilisable jusqu'à son expiration naturelle (30 jours).
    // On ignore silencieusement l'échec : la session locale est de toute façon nettoyée.
    if (refreshToken) {
