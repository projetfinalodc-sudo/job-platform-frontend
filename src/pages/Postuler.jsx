...
import {
  FiArrowLeft, FiUploadCloud, FiFile, FiX, FiSend, FiEdit3, FiPaperclip, FiZap,
} from 'react-icons/fi';
...
function Postuler() {
  ...
  const [generationIaEnCours, setGenerationIaEnCours] = useState(false);
  ...
  const genererLettreParIa = async () => {
    setGenerationIaEnCours(true);
    try {
      const lettre = await aiService.genererLettreMotivation(offre.id);
      setLettreTexte(lettre);
      setModeLettreFichier(false);
      toast.success('Brouillon généré ! Relisez-le et modifiez-le avant d\'envoyer.');
    } catch (error) {
      toast.error(extraireErreur(error).message);
    } finally {
      setGenerationIaEnCours(false);
    }
  };
            ...
            {modeLettreFichier ? (
              ...
              <>
                <button
                  ...
                  onClick={genererLettreParIa}
                  disabled={generationIaEnCours}
                  style={{ marginBottom: '0.75rem' }}
                >
                  <FiZap /> {generationIaEnCours ? 'Génération en cours…' : 'Générer un brouillon avec l\'IA'}
                </button>
                <textarea
                ...
                />
                <p style={{ fontSize: '0.72rem', color: 'var(--slate-400)', marginTop: '0.4rem' }}>
                  Le brouillon généré par IA est une base de départ — relisez-le et personnalisez-le avant l'envoi.
                </p>
              </>
            )}
          </div>
